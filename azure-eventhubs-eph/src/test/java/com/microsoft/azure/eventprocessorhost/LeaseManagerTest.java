...
public class LeaseManagerTest
{
	...
	void singleManagerLeaseSmokeTest(boolean useAzureStorage, int partitionCount) throws Exception
	{
		...
		Boolean boolret = this.leaseManagers[0].leaseStoreExists();
		...
		boolret = this.leaseManagers[0].leaseStoreExists();
		...
		{
			Lease createdLease = this.leaseManagers[0].createLeaseIfNotExists(String.valueOf(i));
		...
		for (int i = 0; i < partitionCount; i++)
		{
			...
			boolret = this.leaseManagers[0].renewLease(leases[i]);
		...
		boolret = this.leaseManagers[0].updateLease(leases[1]);
		assertTrue("failed to update lease for 1", boolret);
		Lease retrievedLease = this.leaseManagers[0].getLease("1");
		...
		boolret = this.leaseManagers[0].renewLease(leases[0]);
		...
		{
			TestUtilities.logConditional(useAzureStorage, "Partition " + i + " state before: " + leases[i].getStateDebug());
			boolret = this.leaseManagers[0].releaseLease(leases[i]);
		...
		boolret = this.leaseManagers[0].deleteLeaseStore();
	...
	void twoManagerLeaseStealingTest(boolean useAzureStorage) throws Exception
	{
		...
		boolret = this.leaseManagers[1].leaseStoreExists();
		assertTrue("second manager cannot see lease store", boolret);

		TestUtilities.log("Creating lease for partition 0");
		Lease mgr1Lease = this.leaseManagers[0].createLeaseIfNotExists("0");
		...
		boolret = this.leaseManagers[1].acquireLease(mgr2Lease);
		...
		mgr2Lease = this.leaseManagers[1].getLease("0");
		assertNotNull("second manager cannot see lease for 0", mgr2Lease);

...
final class ExceptionUtil {
    static Exception toException(ErrorCondition errorCondition) {
        if (errorCondition == null) {
            throw new IllegalArgumentException("'null' errorCondition cannot be translated to EventHubsException");
        }

        if (errorCondition.getCondition() == ClientConstants.TIMEOUT_ERROR) {
            return new EventHubsException(ClientConstants.DEFAULT_IS_TRANSIENT, new TimeoutException(errorCondition.getDescription()));
            ...
            return new EventHubsException(true, new AmqpException(errorCondition));
        } else if (errorCondition.getCondition() == ClientConstants.ARGUMENT_ERROR) {
            return new EventHubsException(false, errorCondition.getDescription(), new AmqpException(errorCondition));
            ...
            return new EventHubsException(false, errorCondition.getDescription());
        ...
        return new EventHubsException(ClientConstants.DEFAULT_IS_TRANSIENT, errorCondition.getDescription());
    }
    ...
    static Exception amqpResponseCodeToException(final int statusCode, final String statusDescription) {
        ...
        if (amqpResponseCode == null)
            return new EventHubsException(true, String.format(ClientConstants.AMQP_PUT_TOKEN_FAILED_ERROR, statusCode, statusDescription));
    ...
    static <T> void completeExceptionally(CompletableFuture<T> future, Exception exception, IErrorContextProvider contextProvider) {
        if (exception != null && exception instanceof EventHubsException) {
            ErrorContext errorContext = contextProvider.getContext();
            ((EventHubsException) exception).setContext(errorContext);
        }

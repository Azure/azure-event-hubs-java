...
public class MessageSender extends ClientEntity implements IAmqpSender, IErrorContextProvider {
    ...
    public static CompletableFuture<MessageSender> create(
        ...
        try {
            ...
            msgSender.linkFirstOpen.completeExceptionally(new EventHubsException(false, "Failed to create Sender, see cause for more details.", ioException));
        }
    ...
    public void onError(final Exception completionException) {
        ...
        if (this.getIsClosingOrClosed()) {
            ...
            final Exception finalCompletionException = completionException == null
                    ? new EventHubsException(true, "Client encountered transient error for unknown reasons, please retry the operation.") : completionException;
                ...
                if (retryInterval == null) {
                    ...
                    try {
                        ...
                        this.cleanupFailedSend(
                                pendingSendWorkItem,
                                new EventHubsException(false, "Send operation failed while scheduling a retry on Reactor, see cause for more details.", ioException));
                    }
                }
            } else if (outcome instanceof Released) {
                this.cleanupFailedSend(pendingSendWorkItem, new OperationCancelledException(outcome.toString()));
            } else {
                this.cleanupFailedSend(pendingSendWorkItem, new EventHubsException(false, outcome.toString()));
            }
        } else {
            if (TRACE_LOGGER.isLoggable(Level.FINE))
                TRACE_LOGGER.log(Level.FINE,
    ...
    private void throwSenderTimeout(final CompletableFuture<Void> pendingSendWork, final Exception lastKnownException) {
        ...
        final boolean isClientSideTimeout = (cause == null || !(cause instanceof EventHubsException));
        final EventHubsException exception = isClientSideTimeout
                ? new TimeoutException(String.format(Locale.US, "%s %s %s.", MessageSender.SEND_TIMED_OUT, " at ", ZonedDateTime.now(), cause))
                : (EventHubsException) cause;
    ...
    protected CompletableFuture<Void> onClose() {
        if (!this.getIsClosed()) {
            try {
                ...
                this.linkClose.completeExceptionally(new EventHubsException(false, "Scheduling close failed. See cause for more details.", ioException));
            }
        }
